...
class CryptKey implements CryptKeyInterface
{
    ...
    public function __construct(string $keyPath, #[SensitiveParameter] protected ?string $passPhrase = null, bool $keyPermissionsCheck = \true)
    {
        ...
        if ($keyPermissionsCheck === \true && \PHP_OS_FAMILY !== 'Windows') {
            ...
            if (in_array($keyPathPerms, ['400', '440', '600', '640', '660'], \true) === \false) {
                trigger_error(sprintf('Key file "%s" permissions are not correct, recommend changing to 600 or 660 instead of %s', $this->keyPath, $keyPathPerms), \E_USER_NOTICE);
            }
        }
    }
    /**
     ...
     */
    private function isValidKey(#[SensitiveParameter] string $contents, #[SensitiveParameter] string $passPhrase) : bool
    {
