...
use Matomo\Dependencies\Oauth2\League\OAuth2\Server\Grant\GrantTypeInterface;
...
use Matomo\Dependencies\Oauth2\League\OAuth2\Server\ResponseTypes\AbstractResponseType;
use Matomo\Dependencies\Oauth2\League\OAuth2\Server\ResponseTypes\BearerTokenResponse;
...
class AuthorizationServer implements EmitterAwareInterface
{
    ...
    public function respondToDeviceAuthorizationRequest(ServerRequestInterface $request, ResponseInterface $response) : ResponseInterface
    {
        foreach ($this->enabledGrantTypes as $grantType) {
            if ($grantType->canRespondToDeviceAuthorizationRequest($request)) {
                return $grantType->respondToDeviceAuthorizationRequest($request)->generateHttpResponse($response);
            }
        }
        throw OAuthServerException::unsupportedGrantType();
    }
    ...
    {
        $this->enabledGrantTypes['urn:ietf:params:oauth:grant-type:device_code']->completeDeviceAuthorizationRequest($deviceCode, $userId, $userApproved);
    }
    /**
