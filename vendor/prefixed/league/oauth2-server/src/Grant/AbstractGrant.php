...
abstract class AbstractGrant implements GrantTypeInterface
{
    ...
    {
        return method_exists($client, 'supportsGrantType') === \false || $client->supportsGrantType($grantType) === \true;
    }
    /**
    ...
    protected function getBasicAuthCredentials(ServerRequestInterface $request) : array
    {
        ...
        $decoded = base64_decode(substr($header, 6), \true);
        if ($decoded === \false) {
            return [null, null];
        }
     ...
     * @throws UniqueTokenIdentifierConstraintViolationException
     */
    protected function issueAuthCode(DateInterval $authCodeTTL, ClientEntityInterface $client, string $userIdentifier, ?string $redirectUri, array $scopes = []) : AuthCodeEntityInterface
    {
        $maxGenerationAttempts = self::MAX_RANDOM_TOKEN_GENERATION_ATTEMPTS;
