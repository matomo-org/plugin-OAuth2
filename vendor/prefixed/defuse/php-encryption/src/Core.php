...
final class Core
{
    ...
    {
        Core::ensureTrue(Core::ourStrlen($ctr) === Core::BLOCK_BYTE_SIZE, 'Trying to increment a nonce of the wrong size.');
        Core::ensureTrue(\is_int($inc), 'Trying to increment nonce by a non-integer.');
        // The caller is probably re-using CTR-mode keystream if they increment by 0.
        Core::ensureTrue($inc > 0, 'Trying to increment a nonce by a nonpositive amount');
        Core::ensureTrue($inc <= \PHP_INT_MAX - 255, 'Integer overflow may occur');
        /*
    ...
    {
        if ($octets <= 0) {
            throw new Ex\CryptoException('A zero or negative amount of random bytes was requested.');
        }
        self::ensureFunctionExists('random_bytes');
        try {
            return \random_bytes(max(1, $octets));
        } catch (\Exception $ex) {
            throw new Ex\EnvironmentIsBrokenException('Your system does not have a secure random number generator.');
        }
    }
    /**
    ...
    public static function HKDF($hash, $ikm, $length, $info = '', $salt = null)
    {
        ...
        $digest_length = Core::ourStrlen(\hash_hmac($hash, '', '', \true));
        // Sanity-check the desired output length.
        Core::ensureTrue(!empty($length) && \is_int($length) && $length >= 0 && $length <= 255 * $digest_length, 'Bad output length requested of HDKF.');
    ...
    {
        Core::ensureTrue(\defined($name), 'Constant ' . $name . ' does not exists');
    }
    /**
    ...
    {
        Core::ensureTrue(\function_exists($name), 'function ' . $name . ' does not exists');
    }
    /**
    ...
    public static function ourSubstr($str, $start, $length = null)
    {
        ...
        if ($exists) {
            ...
            if (Core::ourStrlen($substr) > $length || Core::ourStrlen($substr) === 0 && $length !== 0 && $start !== $input_len) {
                throw new Ex\EnvironmentIsBrokenException('Your version of PHP has bug #66797. Its implementation of
    ...
    public static function pbkdf2($algorithm, #[\SensitiveParameter] $password, $salt, $count, $key_length, $raw_output = \false)
    {
        ...
        Core::ensureTrue(\in_array($algorithm, \hash_algos(), \true), 'Invalid or unsupported hash algorithm.');
        // Whitelist, or we could end up with people using CRC32.
        $ok_algorithms = ['sha1', 'sha224', 'sha256', 'sha384', 'sha512', 'ripemd160', 'ripemd256', 'ripemd320', 'whirlpool'];
        Core::ensureTrue(\in_array($algorithm, $ok_algorithms, \true), 'Algorithm is not a secure cryptographic hash function.');
        ...
        $hash_length = Core::ourStrlen(\hash($algorithm, '', \true));
