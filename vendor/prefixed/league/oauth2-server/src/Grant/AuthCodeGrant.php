...
use Matomo\Dependencies\OAuth2\League\OAuth2\Server\CodeChallengeVerifiers\CodeChallengeVerifierInterface;
use Matomo\Dependencies\OAuth2\League\OAuth2\Server\CodeChallengeVerifiers\PlainVerifier;
use Matomo\Dependencies\OAuth2\League\OAuth2\Server\CodeChallengeVerifiers\S256Verifier;
...
use Matomo\Dependencies\OAuth2\League\OAuth2\Server\ResponseTypes\RedirectResponse;
...
class AuthCodeGrant extends AbstractAuthorizeGrant
{
    ...
    public function respondToAccessTokenRequest(ServerRequestInterface $request, ResponseTypeInterface $responseType, DateInterval $accessTokenTTL) : ResponseTypeInterface
    {
        ...
        try {
            ...
            $scopes = $this->scopeRepository->finalizeScopes($this->validateScopes($authCodePayload->scopes), $this->getIdentifier(), $client, $authCodePayload->user_id, $authCodePayload->auth_code_id);
        ...
        if (!isset($authCodePayload->code_challenge) && $codeVerifier !== null) {
            throw OAuthServerException::invalidRequest('code_challenge', 'code_verifier received when no code_challenge is present');
        }
    ...
    private function validateCodeChallenge(object $authCodePayload, ?string $codeVerifier) : void
    {
        ...
        if (property_exists($authCodePayload, 'code_challenge_method')) {
            if (isset($this->codeChallengeVerifiers[$authCodePayload->code_challenge_method])) {
                ...
                if (!property_exists($authCodePayload, 'code_challenge') || !isset($authCodePayload->code_challenge) || $codeChallengeVerifier->verifyCodeChallenge($codeVerifier, $authCodePayload->code_challenge) === \false) {
     ...
     */
    private function validateAuthorizationCode(stdClass $authCodePayload, ClientEntityInterface $client, ServerRequestInterface $request) : void
    {
    ...
    public function validateAuthorizationRequest(ServerRequestInterface $request) : AuthorizationRequestInterface
    {
        ...
        } elseif ($client->getRedirectUri() === '' || is_array($client->getRedirectUri()) && count($client->getRedirectUri()) !== 1) {
            $this->getEmitter()->emit(new RequestEvent(RequestEvent::CLIENT_AUTHENTICATION_FAILED, $request));
            throw OAuthServerException::invalidClient($request);
        }
        $stateParameter = $this->getQueryStringParameter('state', $request);
        $scopes = $this->validateScopes($this->getQueryStringParameter('scope', $request, $this->defaultScope), $this->makeRedirectUri($redirectUri ?? $this->getClientRedirectUri($client), $stateParameter !== null ? ['state' => $stateParameter] : []));
        ...
        if ($codeChallenge !== null) {
            ...
            if ($codeChallengeMethod === null) {
                throw OAuthServerException::invalidRequest('code_challenge_method', 'Code challenge method must be provided when `code_challenge` is set.');
            }
            if (array_key_exists($codeChallengeMethod, $this->codeChallengeVerifiers) === \false) {
                throw OAuthServerException::invalidRequest('code_challenge_method', 'Code challenge method must be one of ' . implode(', ', array_map(function ($method) {
                    return '`' . $method . '`';
            ...
            if (preg_match('/^[A-Za-z0-9-._~]{43,128}$/', $codeChallenge) !== 1) {
                throw OAuthServerException::invalidRequest('code_challenge', 'Code challenge must follow the specifications of RFC-7636.');
            }
    ...
    public function completeAuthorizationRequest(AuthorizationRequestInterface $authorizationRequest) : ResponseTypeInterface
    {
        ...
        $finalRedirectUri = $authorizationRequest->getRedirectUri() ?? $this->getClientRedirectUri($authorizationRequest->getClient());
